...
import { showActionSheet } from '../../components/Feedback';
...
export default function AlertasScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchNearbyPets(location.latitude, location.longitude, radius);
      setPets(data);
    } catch {
      // silencioso
    } finally {
  ...
  const { counts, list } = useMemo(() => {
    ...
    const c = colorText.trim().toLowerCase();
    const base = pets.filter((p) => {
      ...
      if (c && !(p.color ?? '').toLowerCase().includes(c)) return false;
      return true;
    });
      ...
      <TouchableOpacity style={styles.card} activeOpacity={0.85} onPress={() => openItem(item)}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.cardMetaRow}>
            ...
            <Text style={styles.cardMeta}>{Math.round(item.distance)}m</Text>
            {item.lost_date && (
              <>
  ...
  return (
    <View style={styles.container}>
      ...
      {loading && pets.length === 0 ? (
        <View style={styles.empty}><ActivityIndicator size="large" color="#FF4757" /></View>
      ) : (
        <FlatList
          data={list}
          keyExtractor={(p) => p.id}
...
const styles = StyleSheet.create({
  ...
  emptyText: { fontSize: 14, color: '#A4B0BE', fontWeight: '600' },
});
